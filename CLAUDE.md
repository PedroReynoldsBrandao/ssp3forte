...
**Blog articles section** (`#artigos`) — a grid of articles rendered by JS, shown in **all four languages**. PT and BR draw from `BLOG_POSTS` (the Portuguese blog, problemasnaprostata.blogspot.com, pool of 16); EN and INT draw from `BLOG_POSTS_EN` (the English blog, **bph-prostate-enlarged.blogspot.com**, pool of 9). The three grids are empty containers `#blog-grid-pt` / `#blog-grid-br` / `#blog-grid-en`, all filled on load by `renderBlogCards()`; CSS shows only the active language’s. EN entries carry `tp`/`ep` only (no PT/BR split). The English blog's 24 legacy posts have no editorial photography at all — the only image on them is the product bottle, repeated — so those cards all take a drawn cover. Only the three articles written in September 2026 carry real photographs, and two of the three have one. The card CTA is "Read article" for EN, "Ler artigo" otherwise.
...
**The three English articles published 8 Sep 2026** — *PSA total and free PSA*, *Belly fat and prostate health*, *Vitamin D and the prostate* — were written for this site and published to the blog under Pedro's author account. They exist because **none of the 24 legacy English posts passes the curation criterion**: every one of them closes on a product pitch, and several carry claims far stronger than anything on the site ("The best Doctors in EUROPE prescribe this natural miracle to 80% of their prostate patients", "90% improvement in symptoms in 8-12 weeks", "SSP3-Forte can heals Naturally, there absolutely NO side effects"). The new three name no product, make no health claim, and close by pointing the reader at their doctor. Keep that standard for anything added to the English blog. Their photographs are Naturalfarma's own, reused from the Portuguese blog.

**Access limits on the English blog:** `pedrorbrandao@gmail.com` is an **author**, not an administrator. That is enough to write and publish new posts, but not to change the theme, edit the layout, or touch the 24 posts authored by "Nutrition for Good". The blog still runs a 2011-era template, and `What you must know about prostate problems` has images pointing at `file:///C:/Users/Pedro/AppData/...` that are broken for every visitor. Both need the blog admin to promote the account in **Definições → Permissões → Autores do blogue**.
...
**Info modal** (`#info-modal`) — floating overlay, PT/BR only. Triggered from hero "Saber mais" dropdown and pricing section button. Rewritten Feb 2026 with copy supplied by the client, applied verbatim in substance and adapted to each variant’s orthography. Four sections: benefits from week 6, how to use it, a comparison against pharmaceutical treatments, and lifestyle advice. The old "irá regularizar os PSA … e prevenir a biópsia e a cirurgia" claim is gone, as is the claim that the product contains selenium (it is not among the 8 ingredients) and the old dosing table by prostate weight. The remaining copy still makes unauthorised health claims under Regulation (EC) 1924/2006 (PSA, prostate size, urinary flow, nocturia, erection, spermatogenesis) and presents the product as a "remédio"/"tratamento" for a named disease — this was put to the client in writing and applying it as written was their decision. Older source text: `O que o SSP3-Forte pode fazer por Você.txt`.
...
The `.sv-fine` block at the end carries everything else in 0.68rem grey: the 290 base, the 223 excluded questionnaires, the 81.3% across all respondents, the blank-field caveat behind "no medication", post-hoc selection with p=0.106, survivorship bias, the absence of a control group or baseline, and Regulation (EC) 1924/2006. **Do not quote "88%" elsewhere on the site** — the figure for all respondents is 81.3%.
