...
| `tools/gen_blog_index.py` | Writes the crawlable all-articles list into `index.html` from the pools |
...
**All-articles list (`.blog-index`)** — under each grid, a plain list of *every*
article in that language's pool, generated by `tools/gen_blog_index.py` between
`<!-- BLOG INDEX xx -->` markers. Run it after adding an article to either pool;
it reads `BLOG_POSTS` / `BLOG_POSTS_EN` straight out of `index.html`, so there are
no titles duplicated by hand, and it is idempotent.

It exists for crawlability, not decoration. The grid above it is built by JS into
an empty `<div>`, so the article links do not appear in the HTML Googlebot reads
first — only the five links to the blog *home pages* did. The weekly rotation made
it worse: each article dropped out of the markup for weeks at a time. **In Sep 2026
the seven new English articles sat in "Crawled – currently not indexed" in Search
Console**, one of the seven aside; the dominant cause is that the blog has no
authority after years dormant, but the missing links were the part fixable from
this repo. Keep these links visible and real — hidden link lists are worth less and
read as manipulation.

For the record, the same Search Console report showed 10 pages as "Alternate page
with proper canonical tag" (Blogger's `?m=1` mobile variants) and 2 as "Page with
redirect" (old `http://` URLs). Both are correct behaviour and need no action.
